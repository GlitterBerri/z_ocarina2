/*
 * Do not edit this file_  It was automatically generated
 * by "rgb2c" from the file "font_e_2F_int"_
 *
 *   Size: 16 x 16
 *   Number of channels: 3
 *   Number of bits per texel: 4 (G_IM_SIZ_4b)
 *   Format of texel: G_IM_FMT_I
 *
 * Example usage:
 *
 *   gsDPPipeSync (),
 *   gsDPSetCombineMode (G_CC_DECALRGB, G_CC_DECALRGB),
 *   gsDPSetTexturePersp (G_TP_PERSP),
 *   gsDPSetTextureLOD (G_TL_TILE),
 *   gsDPSetTextureFilter (G_TF_BILERP),
 *   gsDPSetTextureConvert(G_TC_FILT),
 *   gsDPSetTextureLUT (G_TT_NONE),
 *   gsDPLoadTextureBlock_4b (font_e_2F_txt, G_IM_FMT_I, 16, 16, 0,
 *     G_TX_WRAP | G_TX_NOMIRROR, G_TX_WRAP | G_TX_NOMIRROR,
 *     G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD),
 */

#if 0	/* Image preview */
	+----------------+
	|______~,________|
	|_____,%,________|
	|_____+*_________|
	|_____*+_________|
	|____,%,_________|
	|____+*__________|
	|____*+__________|
	|___,%,__________|
	|___+*___________|
	|___*o___________|
	|___%,___________|
	|__~*____________|
	|__*o____________|
	|__o,____________|
	|________________|
	|________________|
	+----------------+
#endif

// Gfx font_e_2F_txt_C_dummy_aligner1[] = { //EndDisplayList() };

unsigned char font_e_2F_txt[] = {
	0x0, 0x0, 0x0, 0x52, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0x2, 0xd3, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0x6, 0xb0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0xa, 0x70, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0x2c, 0x20, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0x6b, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0xa7, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x2, 0xc3, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x6, 0xb0, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0xa, 0x80, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x1c, 0x30, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x5b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0xa8, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
	0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 
};

